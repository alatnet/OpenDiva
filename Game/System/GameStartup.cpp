...
{
    static char gameBuffer[sizeof(LYGame::OpenDivaGame)];

    ISystem* system = m_Framework->GetISystem();
    ModuleInitISystem(system, "OpenDiva");

    m_Game = new(reinterpret_cast<void*>(gameBuffer)) LYGame::OpenDivaGame();
...
{
	/*const bool requireUserLogin = GetISystem()->GetGemManager()->IsGemEnabled(AZ::Uuid("c9c25313197f489a8e9e8e6037fc62eb"), { ">=0.1.0" });
	ExecuteAutoExec(requireUserLogin);*/
#ifdef WIN32
...
void GameStartup::ExecuteAutoExec(bool requireUserLogin /*= true*/) {
	/*if (m_bExecutedAutoExec) return;
	...
	if (!requireUserLogin || userCount > 0) {
