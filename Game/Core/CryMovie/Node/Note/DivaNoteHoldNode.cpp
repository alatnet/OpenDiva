...
namespace LYGame {
	DivaNoteHoldNode::DivaNoteHoldNode(ResourceCollection *rc, IDivaJudge *divaJudge, NoteEntryHold* entry/*, NoteEntryBPM bpm*/) : DivaNoteBaseNode(rc, divaJudge), m_NoteEntry(entry) {
		...
		this->m_currPos1 = this->m_currPos2 = Vec2(0, 0);
		...
		if (enote->effects.size() > 0) { //if we have effects
			...
			for (int i = 0; i < numENoteEffects; i++) { //for each effect
				...
				for (int aI = 0; aI < entry.anim.size(); aI++) { //for each animation
					...
					track->posTrack->SetValue(start2 + anim.time, { anim.pos.x, anim.pos.y, 0 }); //set the position based on time
	...
	void DivaNoteHoldNode::Render() {
		if (this->m_renderOut && this->m_holdEnabled) {
			...
			if (this->m_holdMult > 0) {
				float ypos = this->m_NoteEntry->pos.y - this->m_pRC->p_FontResource->getMultiSize().y;

				if (m_hitscoreWrong) ypos -= (this->m_pRC->p_FontResource->getSize(MISSRATING(m_hitScore)).y);
				else  ypos -= (this->m_pRC->p_FontResource->getSize(m_hitScore).y);

				this->m_pRC->p_FontResource->DrawMult({ this->m_NoteEntry->pos.x,  ypos}, 10 * this->m_holdMult);
			}
	...
	//Input
	void DivaNoteHoldNode::OnCross(int mode, float value) {
	...
	void DivaNoteHoldNode::OnCircle(int mode, float value) {
		CheckRange(eNT_Circle, face, arrow)
	}

	void DivaNoteHoldNode::OnSquare(int mode, float value) {
		CheckRange(eNT_Square, face, arrow)
	}

	void DivaNoteHoldNode::OnTriangle(int mode, float value) {
		CheckRange(eNT_Triangle, face, arrow)
	}

	//fix this shit?
	void DivaNoteHoldNode::OnLeft(int mode, float value) {
		CheckRange(eNT_Square, arrow, face)
	}

	void DivaNoteHoldNode::OnRight(int mode, float value) {
		CheckRange(eNT_Circle, arrow, face)
	}

	void DivaNoteHoldNode::OnUp(int mode, float value) {
		CheckRange(eNT_Triangle, arrow, face)
	}

	void DivaNoteHoldNode::OnDown(int mode, float value) {
		CheckRange(eNT_Cross, arrow, face)
	}

	void DivaNoteHoldNode::OnSwipeL(int mode, float value) {
		if (!this->m_faceHeld && !this->m_arrowHeld && this->m_holdEnabled) {
			IDivaJudgeParams params;
			params.hold = true;
			params.holdRelease = true;
			...
			if (this->m_bTime1 >= m_pDivaJudge->m_Range_CoolF) params.hitscore = eHS_Worst;
			if (this->m_bTime1 <= m_pDivaJudge->m_Range_CoolF && this->m_bTime1 > m_pDivaJudge->m_Range_WorstF) params.hitscore = eHS_Worst;
	...
	void DivaNoteHoldNode::OnSwipeR(int mode, float value) {
		if (!this->m_faceHeld && !this->m_arrowHeld && this->m_holdEnabled) {
			IDivaJudgeParams params;
			params.hold = true;
			params.holdRelease = true;
	...
	void DivaNoteHoldNode::OnStar(int mode, float value) {
		if (!this->m_faceHeld && !this->m_arrowHeld && this->m_holdEnabled) {
			IDivaJudgeParams params;
			params.hold = true;
			params.holdRelease = true;
	...
	void DivaNoteHoldNode::DrawNoteE2(Vec2 pos, ENoteType type) {
		if (this->m_pENoteTracks2.size() > 0) { //if we have effects
			for (ENoteTrack * track : this->m_pENoteTracks2) { //for each effect
				if (track->img == nullptr) //we are the enote data
					this->m_pRC->p_NoteResource->getNote(type)->DrawNoteE({ pos.x + track->pos.x, pos.y + track->pos.y }, this->m_timeArrow2, track->sro.x, track->sro.y, track->sro.z); //draw the enote
