...
namespace LYGame {
	...
	DivaNoteBaseNode* DivaNoteNodeFactory::newNoteNode(std::string name, ResourceCollection *rc, IDivaJudge *divaJudge, NoteEntry* entry) {
		auto funcEntry = this->m_funcs->find(name);
		if (funcEntry != this->m_funcs->end()) return funcEntry->second(rc, divaJudge, entry);
