...
namespace LYGame {
	DivaNoteSingleNode::DivaNoteSingleNode(ResourceCollection *rc, NoteEntrySingle* entry/*, NoteEntryBPM bpm*/) : DivaNoteBaseNode(rc), m_NoteEntry(entry) {
		this->m_pBeizerTrack = NULL;
		this->m_currPos = AZ::Vector2(0, 0);
		this->m_bTime = 0.0f;
	...
	if (mode == eIET_Pressed && !this->m_noteHit) { \
		IDivaJudgeParams params; \
		params.nType = this->m_NoteEntry->type; \
		params.sType = this->m_NoteEntry->sType; \
		params.notePos = this->m_NoteEntry->pos; \
		params.hType = eNHT_Single; \
		DivaJudgeBus::BroadcastResult(params.hitscore, &DivaJudgeBus::Events::GetHitScore, this->m_bTime, eHS_None); \
	...
	//Input
	void DivaNoteSingleNode::OnCross(LyInputEventType mode, float value) {
	...
	void DivaNoteSingleNode::OnCircle(LyInputEventType mode, float value) {
		CheckRange(eNT_Circle)
	}

	void DivaNoteSingleNode::OnSquare(LyInputEventType mode, float value) {
		CheckRange(eNT_Square)
	}

	void DivaNoteSingleNode::OnTriangle(LyInputEventType mode, float value) {
		CheckRange(eNT_Triangle)
	}

	void DivaNoteSingleNode::OnLeft(LyInputEventType mode, float value) {
		this->OnSquare(mode, value);
	}

	void DivaNoteSingleNode::OnRight(LyInputEventType mode, float value) {
		this->OnCircle(mode, value);
	}

	void DivaNoteSingleNode::OnUp(LyInputEventType mode, float value) {
		this->OnTriangle(mode, value);
	}

	void DivaNoteSingleNode::OnDown(LyInputEventType mode, float value) {
		this->OnCross(mode, value);
	}

	void DivaNoteSingleNode::OnSwipeL(LyInputEventType mode, float value) {
	...
	void DivaNoteSingleNode::OnSwipeR(LyInputEventType mode, float value) {
	...
	void DivaNoteSingleNode::OnStar(LyInputEventType mode, float value) {
		//CheckRange(eNT_Star)
		if (mode == eIET_Pressed && !this->m_noteHit) {
			IDivaJudgeParams params;
			params.nType = this->m_NoteEntry->type;
