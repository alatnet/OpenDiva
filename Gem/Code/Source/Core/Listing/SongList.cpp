...
namespace OpenDiva {
	long long SongList::m_groups = 0;
	long long SongList::m_songs = 0;
	AZStd::mutex SongList::m_mutex;

	void SongList::Refresh() {
		SongList::m_mutex.lock();
		SongList::m_groups = 0;
		SongList::m_songs = 0;
		SongList::m_mutex.unlock();
	...
	void SongList::ProcessGroup(AZStd::string path, AZStd::string dbcrc, SQLite3::SQLiteDB * sysDb) {
		...
		AZStd::string guuid_str = Util::Crc32ToString(guuid);
	...
	void SongList::ProcessSong(AZStd::string path, AZStd::string guuid, AZStd::string dbcrc, SQLite3::SQLiteDB * sysDb) {
		...
		AZStd::string suuid = Util::Crc32ToString(suuid_hash);
				...
				if (info.hash != AZ_CRC("NotValid", 0x072bdcd9)) {
					...
					AZStd::string songname_sql_str =
						"INSERT OR REPLACE INTO SongNames ("
							...
							"suuid"
						") VALUES ("
							"\"" + Util::Crc32ToString(snuuid_hash) + "\","
							"\"" + info.name + "\","
							"\"" + info.desc + "\","
							"\"" + authors + "\","
							"\"" + info.lang + "\","
				...
				if (info.hash != AZ_CRC("NotValid", 0x072bdcd9)) {
					AZStd::string notemap_sql_str =
						"INSERT OR REPLACE INTO Notemaps ("
							...
							"suuid"
						") VALUES ("
							"\"" + Util::Crc32ToString(nmuuid_hash) + "\","
							"\"" + fileStr + "\","
							"\"" + fileNameStr + "\","
							"\"" + info.author + "\","
							"\"" + info.desc + "\","
